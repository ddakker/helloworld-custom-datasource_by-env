...
function inject_datasource() {
  ...
  if [ -z "${driver}" ] && [ -n "${db}" ]; then
    ...
    if [ "${db}" = "MYSQL" ] || [ "${db}" = "ORACLE" ] || [ "${db}" = "POSTGRESQL" ]; then
      driver="${db,,}"
    fi
  fi
  ...
  case "${driver}" in
      ...
      ;;
      "oracle")
	      map_properties "jdbc:oracle" "${prefix}_XA_CONNECTION_PROPERTY_ServerName" "${prefix}_XA_CONNECTION_PROPERTY_Port" "${prefix}_XA_CONNECTION_PROPERTY_DatabaseName" "invalid"
        if [ "${validate}" = "false" ]; then
          validate="true"
          checker="org.jboss.jca.adapters.jdbc.extensions.oracle.OracleValidConnectionChecke"
